...
void Proc_Init(void)
{
    ...
    if (g_CurProcessDriver->adapter != BL_NULL_PTR)
    {
        (void)g_CurProcessDriver->adapter->Init();
    }
}

#if (PROC_NUMBER_OF_DRIVER > 1u)
/**************************************************************************//**
 *
...
bl_Return_t Proc_ActivateDriver(bl_u16_t id)
{
    ...
    {
        g_CurProcessDriver = &(g_ProcessDriverList.procList[id]);
...
void Proc_Timeout(void)
{
    if ((g_CurProcessDriver->adapter != BL_NULL_PTR))
    {
        g_CurProcessDriver->adapter->Timeout();
    }
}
...
void Proc_Sleep(void)
{
    if ((g_CurProcessDriver->adapter != BL_NULL_PTR))
    {
        g_CurProcessDriver->adapter->Sleep();
    }
}
