...
#define DET_Err_TimerNotMount	(DET_Err_CanNotWritePinHDvr+1U)
...
#define DET_Err_TimerNotRuning	(DET_Err_TimerCanNotChg+1U)
#define DET_Err_PPGmasterMustBeOdd	(DET_Err_TimerNotRuning+1U)
#define DET_Err_PPGmasterSelect	(DET_Err_PPGmasterMustBeOdd+1U)
...
#define DET_Err_PpgNotInitial	(DET_Err_PpgMultMasterFreqCanNotChg+1U)
...
#define DET_Err_DMAIsBusy		(DET_Err_DMAIDOutOfRange+1U)
...
#define DET_Err_BufferOverLap	(DET_Err_SerialParaInvalib+1u)
...
#define DET_Err_NCV7728_NULL				(DET_Err_M93CXX_Len+1u)
#define DET_Err_NCV7728_ParaOutOfRange	(DET_Err_NCV7728_NULL+1u)
#define DET_Err_NCV7728_ChannleOutOfRange	(DET_Err_NCV7728_ParaOutOfRange+1u)
#define DET_Err_NCV7728_DrvNotReady		(DET_Err_NCV7728_ChannleOutOfRange+1u)
#define DET_Err_NCV7728_DrvFatalErr			(DET_Err_NCV7728_DrvNotReady+1u)

#define DET_Err_NCV7240_ParaOutOfRange		(DET_Err_NCV7728_DrvFatalErr+1u)
...
/*###############################################################
		for Error monitor
###############################################################*/
#define DET_HaltWhenErrorHappen	0u

#if DET_HaltWhenErrorHappen !=0u
	uint16 DET_u16FatalErr(int16 i16ErrLineNum, uint16 u16ErrID);	

	#define DET_ErrCheck(TrueCondition,ErrType)	do{if(!(TrueCondition)){u16RetVal=DET_u16FatalErr(__LINE__,(uint16)(ErrType));}}while(0)
	#define DET_ErrReport(ErrType)				do{u16RetVal=DET_u16FatalErr(__LINE__,(uint16)(ErrType));}while(0)
#else
	#define DET_ErrCheck(TrueCondition,ErrType)	do{if(!(TrueCondition)){u16RetVal=ErrType;}}while(0)
	#define DET_ErrReport(ErrType)				do{u16RetVal=ErrType;}while(0)
#endif
