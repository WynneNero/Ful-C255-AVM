...
#define R_FDL_TIMEOUT_CC_CLK_P_LOOP             (3u)
/* timeout loop_count: (40us * f_MHz) / (clk per loop)   ---  but rounding up to be considered */
#define R_FDL_TIMEOUT_CC  (uint32_t)(   (uint32_t)( (40u * g_fdl_str.RTCfg_pstr->cpuFrequencyMHz_u16) + (R_FDL_TIMEOUT_CC_CLK_P_LOOP - 1u) ) / R_FDL_TIMEOUT_CC_CLK_P_LOOP )

/* IO register check loop timeout */
/* clocks per timeout loop */
#define R_FDL_TIMEOUT_CHKREG_CLK_P_LOOP         (19u)
/* timeout loop_count: (40us * f_MHz) / (clk per loop)   ---  but rounding up to be considered */
#define R_FDL_TIMEOUT_CHKREG  (uint32_t)(   (uint32_t)( (40u * g_fdl_str.RTCfg_pstr->cpuFrequencyMHz_u16) + (R_FDL_TIMEOUT_CHKREG_CLK_P_LOOP - 1u) ) / R_FDL_TIMEOUT_CHKREG_CLK_P_LOOP )
