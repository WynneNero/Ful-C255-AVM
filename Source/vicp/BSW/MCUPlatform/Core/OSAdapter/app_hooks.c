...
#if (OS_CPU_HOOKS_EN > 0u) && (OS_TASK_STAT_STK_CHK_EN > 0u)
extern void  OSErrorHook (U8 task, U8 Error);
...
void  App_TaskStatHook (void)
{
   ...
   OS_TCB*   ptcb = OSTCBPrioTbl[prio];
    ...
    {
      prio=OSCfg_astAllTask[i].taskpriority;
      ptcb=OSTCBPrioTbl[prio];
      if(ptcb->OSTCBStkUsed==ptcb->OSTCBStkSize)
      {
	OSErrorHook(prio,OS_STACK_OV);
      }
    }
#endif    
}
...
{
   INT8U prio=p_tcb->OSTCBPrio;

   	
  if((prio>=OSCfg_astAllTask[0].taskpriority)&&
  	(prio<OSCfg_astAllTask[OSAPI_TASKNUM-1U].taskpriority))
  {
    OSErrorHook(prio,OS_QUIT);
   }
}
