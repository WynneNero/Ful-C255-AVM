...
void gpio86_test(void)
{
	...
	{
		GPIO_SetOutputMode(GPIO_PORT_8,GPIO_PIN_6,GPIO_OPT_INIT,GPIO_OPT_INIT);
		GPIO_SetVal(GPIO_PORT_8,GPIO_PIN_6,FALSE);
	}
	else if(test_86==3)
	{
...
void Open_SOC_Power(void)
{
	...
	POWER_SOC_LOW_ON(); 
	POWER_5V0_ON(); //PS power
    GPIO_SetVal(GPIO_PORT_0,GPIO_PIN_4,TRUE);
	POWER_PMIC_ON();
    //GPIO_SetVal(GPIO_PORT_8,GPIO_PIN_6,TRUE);
//	Timer_Start(OPEN_FPD_LINK_1V1,3,TIMER_ONE_SHOT);
  	CS_SIG_SyncSend(SIG_B_SOC_POWER_ON, &NaviPowerStatus);
}
//void Open_FPD_LINK_1V1(void)
//{
//    POWER_FPD_LINK_1V1_ON();
//    Timer_Start(OPEN_FPD_LINK_1V8,3,TIMER_ONE_SHOT);  //t1=2ms
//}

//void Open_FPD_LINK_1V8(void)
//{
//    POWER_FPD_LINK_1V8_ON();
//    Timer_Start(OPEN_CAMERA_5V0,6,TIMER_ONE_SHOT); // t2=5ms
//}
...
{
	POWER_PMIC_ON();	
	Timer_Start(OPEN_M4UART_PWR,450,TIMER_ONE_SHOT);   // uidq0888:for EE series of electric
}

/***********************************************************************
*  Name        : CheckCameraPower
*  Description : Check Camera Power status        
*  Parameter   : None
*  Returns     : None
***********************************************************************/
void CheckCameraPower(void)
{
	U8 is_camera_power_on = 0;
	static U8 resetCloseFlagCnt = 0;
	CS_SIG_SyncReceive(SIG_CAMERA_ONOFF,&is_camera_power_on);
 	if(CAMERA_CTRL_COMPLETE==get_camera_power_step())
	{
		//check camera power error(LM53601)
		if(CHECK_LM53601_RESET_PIN_FLAG)   
		{
			// do nothing
		}
		else 
		{
			if(resetCloseFlagCnt++ >= 20)
			{
				Close_CAM_Power();
...
/***********************************************************************
*  Name        : SocControlCameraPower
*  Description : SOC Control Camera Power switch        
*  Parameter   : None
*  Returns     : None
***********************************************************************/
void SocControlCameraPower(void)
{
	...
	if(IS_SOC_POWERON)
	{
		...
		if(SocPowerDelay300msCnt >= DELAY300MSCNT)
		{	
			
		GPIO_SetInputMode( GPIO_PORT_10, GPIO_PIN_E, GPIO_OPT_INIT);
//			if(0)
			if(GPIO_GetVal(GPIO_PORT_10, GPIO_PIN_E))
			{
...
void Close_SOC_Power_Manage(void)
{
    POWER_SOC_LOW_OFF();
...
BOOL SocDriver_pwmcontrol(U8 channel, U8 duty ,BOOL b_is_enable)
{
    ...
    if(b_is_enable)   
    {
        if(!PWMD_Config(channel,100, duty))   
        {
            return ret= FALSE;
        }
    }
...
{
	if((GPIO_GetVal(GPIO_PORT_8, GPIO_PIN_0))&&(!soc_slc_active))
	{
		soc_slc_sinal_cnt++;
